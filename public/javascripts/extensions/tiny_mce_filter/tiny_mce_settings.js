tinyMCE.init({
    ...
    // theme_advanced_buttons2 : "undo,redo,separator,cut,copy,paste,pastetext,pasteword,separator,search,replace,selectall,separator,link,unlink,image,radiantflash,radiantvideo,separator,visualaid,removeformat,cleanup,code,preview,styleprops",
    ...
    +"object[class|classid|codebase|data|id|onclick|ondblclick|onkeydown"
      +"|onkeypress|onkeyup|onmousedown|onmousemove|onmouseout|onmouseover"
      +"|onmouseup|style|title|type|width|height],"
...
function text_input_method(index, filter) {
  if (index != null) {
   ...
   if (/^snippet_content$/.test(index)) {
     elem_selector = index;
   } else {
     elem_selector = 'part_' + index + '_content';
   }
   elem = $(elem_selector);
   // setBucketAction(elem, filter);
   if (filter == "TinyMce") {
     tinyMCE.execCommand('mceAddControl',true,elem_selector);
   } else {
     tinyMCE.execCommand('mceRemoveControl',true,elem_selector);
   }
  } else {
